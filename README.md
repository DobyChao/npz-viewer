...
填 SSH 用户名 / 主机 / 端口和远端目录，然后点「连接」。连接会做四件事（都幂等）：

1. 用 `rsync` 把后端代码同步到远端；
2. 在远端建 venv、装依赖（优先 Python 3.14）；
3. 启动后端（远端有 `tmux` 就用 tmux，否则用 `nohup`）；
4. 建立 SSH 隧道，并做健康检查。

之后前端的 `/api` 会自动转发到所选后端——**原始 npz 不过网，只有渲染好的图和 JSON 回传**。可随时
在同一面板切换「本机 / 各服务器」，或点「停止」断开隧道并停掉远端后端。顶栏的后端按钮会显示当前
生效的后端。

要求：本机装了 `ssh` 和 `rsync`，且到服务器免密登录（ssh-agent 或密钥）；服务器上有 `python3`
（3.13+ 才能用视频导出）。每台服务器有各自的 `roots.json` 和缓存目录，顶栏「管理 root」增删的是
**所连后端**上的目录。服务器列表存在本机的 `servers.json`（已在 `.gitignore` 中）。

实现上，控制面在 Vite dev server 里（见 `frontend/hub/`）：`/__hub/*` 是本机的管理接口，`/api/*`
被动态反代到当前后端（本机后端或 SSH 隧道）。因此这套能力用于 `npm run dev` 的开发模式。
